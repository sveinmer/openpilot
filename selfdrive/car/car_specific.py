...
class CarSpecificEvents:
  ...
  def update(self, CS: car.CarState, CS_prev: car.CarState, CC: car.CarControl):
    ...
    if self.CP.brand == 'chrysler':
      ...
      # T2A Item 10: route Tesla Pre-AP stock-CC engagement events + pedal calibration gate.
      # Adapted from NotAutopilot/openpilot c0ac6e54e car_specific.py lines 93-108.
      # Upstream gates on self.CP.pcmCruise (= no-pedal mode), but our Path B
      # T2A kept openpilotLongitudinalControl=True/pcmCruise=False hardcoded
      # (Item 6 deferred — see UPSTREAM_DIVERGENCE_2026_05_19.md Errata).
      # Therefore we gate directly on nap_conf.use_pedal instead.
      if self.CP.carFingerprint == "TESLA_MODEL_S_PREAP":
        from opendbc.car.tesla.preap.nap_conf import nap_conf
        if not nap_conf.use_pedal:
          # No-pedal mode: Tesla DI owns longitudinal; surface stalk-engage edges
          # to the driver via alerts. CS fields set in preap/carstate.py.
